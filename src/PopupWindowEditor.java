...
public class PopupWindowEditor extends JFrame {
    ...
    private class PopupEditorPanel extends JPanel implements MouseListener, MouseMotionListener {
        ...
        private void setupToolbar() {
            ...
            JButton previewBtn = new JButton("ดูตัวอย่าง");
            previewBtn.setFont(buttonFont);
            ...
            guidesCheckbox.addActionListener(e -> {
                showGuides = guidesCheckbox.isSelected();
                repaint();
            });
        ...
        private void handleWindowSize() {
            ...
            okBtn.addActionListener(e -> {
                try {
                    ...
                    JOptionPane.showMessageDialog(dialog, "กรุณาป้อนตัวเลขที่ถูกต้อง", "ข้อผิดพลาด",
        ...
        private void handleAddImage() {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                File selectedFile = fileChooser.getSelectedFile();
                String relativePath = getRelativePath(selectedFile);
                MenuElement element = new MenuElement(MenuElement.ElementType.IMAGE, relativePath, config.width / 2,
                        config.height / 2, 0, 0);
        ...
        private void handlePreview() {
            PopupWindow window = new PopupWindow(config, elements);
        ...
        private void handleExportCode() {
            ...
            code.append(String.format("config.backgroundColor = new Color(%d, %d, %d);\n",
                    config.backgroundColor.getRed(), config.backgroundColor.getGreen(),
        ...
        public void parseAndLoadCode(String code) {
            ...
            for (String line : lines) {
                ...
                if (line.contains("config.width =")) {
                    ...
                    if (!elements.isEmpty()
                            && elements.get(elements.size() - 1).getType() == MenuElement.ElementType.TEXT) {
                        try {
                            String colorStr = line.substring(line.indexOf("Color(") + 6, line.indexOf(")"));
                            String[] parts = colorStr.split(",");
                            int r = Integer.parseInt(parts[0].trim());
